...
data_subdir = "data/company_registry"
...
print("Renaming columns in the company registry sheet")
...
#Fix dtypes
print("Fixing critical dtypes (cf, piva, CAP, etc.)")
df_anagrafica['cf'] = df_anagrafica['cf'].astype(str)
df_anagrafica['piva'] = df_anagrafica['piva'].astype(str)
df_anagrafica['indirizzo_cap'] = df_anagrafica['indirizzo_cap'].astype(str)
df_anagrafica['rea'] = df_anagrafica['rea'].astype(str)
print("Dtypes fixed")
...
print("Merging id_impresa back into the main company registry dataset")   
...
#Fix dtypes for critical fields in the codici dataset (cf, rea, loc_n) to ensure they are treated as strings and not misinterpreted as numeric values
print("Fixing dtypes for codici")
df_codici['cf'] = df_codici['cf'].astype(str)
df_codici['rea'] = df_codici['rea'].astype(str)
df_codici['loc_n'] = df_codici['loc_n'].astype(str)

print("Dtypes codici fixed")
...
file_risultati = data_dir + 'imprese_company_registry.csv'
cols_df = pd.read_excel(cols_path, sheet_name='anagrafica',
...
df_anagrafica_repo[cols_to_use].to_csv(  file_risultati, 
                                    ...
                                    encoding='utf-8-sig', 
                                    index=False)
print(f"Saved cleaned company registry data to {file_risultati}")
...
# ============================================================================
# CSV file only headquartiers (also outside FVG)
# ============================================================================
print("Exporting CSV with only headquarters (sedi), including outside FVG")
# - filter the dataset to keep only headquarters records (sede_ul == "SEDE")
# - load the export column mapping and order for Innovation Intelligence
# - sort columns according to the Innovation Intelligence layout
# - export the selected fields to a pipe-delimited CSV file
df_sedi = df_anagrafica[df_anagrafica["sede_ul"] == "SEDE"].copy()

print(f"Dataset sedi ha {df_sedi.shape[0]} righe")
righe_sedi_originali = df_sedi.shape[0]
...
for col in date_cols_sedi:
    if col in df_sedi.columns:
        df_sedi[col + "_dt"] = pd.to_datetime(df_sedi[col], errors="coerce")

date_cols_sedi_dt = [col + "_dt" for col in date_cols_sedi if col + "_dt" in df_sedi.columns]
if date_cols_sedi_dt:
    df_sedi["latest_registry_date"] = df_sedi[date_cols_sedi_dt].max(axis=1)
else:
    df_sedi["latest_registry_date"] = pd.NaT

df_sedi["not_cancelled_first"] = df_sedi["data_canc"].fillna("").eq("").astype(int)
duplicati_sedi = int(df_sedi.duplicated(subset=["cf"], keep=False).sum())

df_sedi = (
    df_sedi.sort_values(
        by=["cf", "latest_registry_date", "not_cancelled_first", "denominazione", "prov", "rea"],
        ascending=[True, False, False, False, True, True],
        na_position="last",
    )
    .drop_duplicates(subset=["cf"], keep="first")
    .copy()
)

cols_cleanup = date_cols_sedi_dt + ["latest_registry_date", "not_cancelled_first"]
df_sedi.drop(columns=cols_cleanup, inplace=True, errors="ignore")

print(
    f"Deduplicated headquarters by cf: removed {righe_sedi_originali - df_sedi.shape[0]} rows across {duplicati_sedi} duplicated records"
)
print(f"Dataset sedi deduplicato ha {df_sedi.shape[0]} righe")

file_risultati = data_dir + 'i2fvg_company_registry_headquarters.csv'
cols_df = pd.read_excel(cols_path, sheet_name='anagrafica',
            usecols = ['nomi_colonne_corretti','ordine_i2fvg']).dropna()
cols_df.sort_values('ordine_i2fvg', inplace=True)
cols_to_use = list(cols_df['nomi_colonne_corretti'])

df_sedi[cols_to_use].to_csv(
    file_risultati,
    sep='|',
    encoding='utf-8-sig',
    index=False
...
print(f"Saved sedi data to {file_risultati}")

print("Exporting CODICI with only headquarters (sedi)")
# Save the CODICI file with only headquarters (including those outside FVG):
# - filter the dataset to keep only headquarters records (loc_n == "0")
# - load the export column mapping and order for Innovation Intelligence
# - sort columns according to the Innovation Intelligence layout
# - export the selected fields to a pipe-delimited CSV file
df_codici_sedi = df_codici[df_codici["loc_n"] == "0"].copy()

file_risultati = data_dir + 'i2fvg_ateco_sedi.csv'
cols_df = pd.read_excel(cols_path, sheet_name='codici',
            usecols = ['nomi_colonne_corretti','ordine_i2fvg']).dropna()
cols_df.sort_values('ordine_i2fvg', inplace=True)
cols_to_use = list(cols_df['nomi_colonne_corretti'])
df_codici_sedi[cols_to_use].to_csv(
    file_risultati,
    sep='|',
    encoding='utf-8-sig',
    index=False
...
print(f"Saved codici sedi data to {file_risultati}")
...
# - sort columns according to the Innovation Intelligence layout
# - export the selected fields to a pipe-delimited CSV file
file_risultati = data_dir + 'i2fvg_company_registry.csv'
cols_df = pd.read_excel(cols_path, sheet_name='anagrafica',
            usecols = ['nomi_colonne_corretti','ordine_i2fvg']).dropna() 
cols_df.sort_values('ordine_i2fvg', inplace = True)
cols_to_use = list(cols_df['nomi_colonne_corretti'])
...
# - sort columns according to the Innovation Intelligence layout
# - export the selected fields to a pipe-delimited CSV file
file_risultati = data_dir + 'i2fvg_ateco.csv'
cols_df = pd.read_excel(cols_path, sheet_name='codici',
            usecols = ['nomi_colonne_corretti','ordine_i2fvg']).dropna() 
cols_df.sort_values('ordine_i2fvg', inplace = True)
cols_to_use = list(cols_df['nomi_colonne_corretti'])
...
print(f"After filtering, company registry dataset has {df_anagrafica.shape[0]} rows") 
# Save the ANAGRAFICA file in the "Innovation Intelligence" version
# after removing local units outside the FVG region
file_risultati = data_dir + 'i2fvg_company_registry_filtered.csv'
cols_df = pd.read_excel(cols_path, sheet_name='anagrafica',
            usecols = ['nomi_colonne_corretti','ordine_i2fvg']).dropna() 
cols_df.sort_values('ordine_i2fvg', inplace = True)
cols_to_use = list(cols_df['nomi_colonne_corretti'])
...
# Save the CODICI file in the "Innovation Intelligence" version
# after removing local units outside the FVG region
file_risultati = data_dir + 'i2fvg_ateco_filtrato.csv'
cols_df = pd.read_excel(cols_path, sheet_name='codici',
            usecols = ['nomi_colonne_corretti','ordine_i2fvg']).dropna() 
cols_df.sort_values('ordine_i2fvg', inplace = True)
cols_to_use = list(cols_df['nomi_colonne_corretti'])
