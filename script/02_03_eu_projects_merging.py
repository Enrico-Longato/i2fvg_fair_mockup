...
base_path = Path.cwd()  
file_company_registry = base_path / "data" / "company_registry" / "i2fvg_company_registry_filtered.csv"

print("Using this file:", file_company_registry)

df_company_registry = pd.read_csv(file_company_registry, sep="|", engine="python", dtype=str)

# keep only active companies (only active companies are shown in I2FVG)
# df_company_registry = df_company_registry[df_company_registry['stato_impresa'].isin(['ATTIVA'])]

filter_i2fvg = df_company_registry[['piva','cf']]
