...
def append_missing_historical_records(current_df: pd.DataFrame, output_path: Path) -> pd.DataFrame:
    """
    Keep records from the previous output if their fiscal code
    is no longer present in the file currently being processed.
    """
    if not output_path.exists():
        print(f"No previous output found for historical merge: {output_path.name}")
        return current_df
    ...
    previous_df.columns = [col.replace("\ufeff", "") for col in previous_df.columns]

    missing_columns = [col for col in current_df.columns if col not in previous_df.columns]
    if missing_columns:
        print(
            f"Skipping historical merge for {output_path.name}: "
            f"missing columns in previous output {missing_columns}"
        )
        return current_df

    identifier_columns = [col for col in ["c fiscale"] if col in current_df.columns]
    if not identifier_columns:
        print(f"Skipping historical merge for {output_path.name}: no identifier column found")
        return current_df

    current_identifiers = {}
    previous_identifiers = {}
    for column in identifier_columns:
        current_values = set(normalize_identifier_series(current_df[column]))
        current_values.discard("")
        current_identifiers[column] = current_values
        previous_identifiers[column] = normalize_identifier_series(previous_df[column])

    missing_mask = pd.Series(True, index=previous_df.index)
    has_any_identifier = pd.Series(False, index=previous_df.index)
    for column in identifier_columns:
        prev_values = previous_identifiers[column]
        non_empty_mask = prev_values != ""
        has_any_identifier = has_any_identifier | non_empty_mask
        missing_mask = missing_mask & (~non_empty_mask | ~prev_values.isin(current_identifiers[column]))

    missing_mask = missing_mask & has_any_identifier
    historical_df = previous_df.loc[missing_mask, current_df.columns].copy()

    if historical_df.empty:
        print(f"No disappeared records to preserve in {output_path.name}")
        return current_df

    historical_df = historical_df.drop_duplicates()
    merged_df = pd.concat([current_df, historical_df], ignore_index=True)
    print(
        f"Preserved {historical_df.shape[0]} disappeared records from "
        f"{output_path.name}"
    )
    return merged_df
...
valid_output = data_path / f"i2fvg_financial.csv"
error_output = data_path / f"i2fvg_financial_errori.csv"

if not df_valid.empty:
    df_valid = append_missing_historical_records(df_valid, valid_output)
