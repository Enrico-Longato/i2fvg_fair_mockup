...
- `01_02_company_registry.py` - Data cleaning and preparation for company registry (produces CSV outputs).
...
- `04_01_django_import_sqlite.py` - Run Django `manage.py import_i2fvg_data` to import data into the SQLite.
...
- `company_registry`: maps original column names from the `FRIULI Anagrafica` sheet to the standardized names used in `01_02_company_registry.py`.
...
```powershell
python main.py --exclude 01_02_company_registry,02_01_eu_projects_download
```
